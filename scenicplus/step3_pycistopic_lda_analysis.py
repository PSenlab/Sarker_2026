...
def load_cistopic_object(filepath):
    ...
    print(f"[OK] Loaded successfully. Shape: {cistopic_obj.fragment_matrix.shape}")
    return cistopic_obj


def save_cistopic_object(cistopic_obj, filepath):
    """Save a cisTopic object to pickle file."""
    print(f"[SAVE] Saving cisTopic object to {filepath}...")
...
def load_lda_models(models_dir):
    ...
    for file_name in model_files:
        ...
        print(f"   [OK] Loaded model: {label}")
    
    print(f"[STATS] Total models loaded: {len(models)}")
    return models
...
def save_region_sets_to_bed(region_dict, output_dir, prefix=""):
    ...
    print(f"[OK] Saved {len(region_dict)} region sets to {output_dir}")
...
def step2_evaluate_models(cistopic_obj):
    ...
    if selected_model is not None:
        print(f"[OK] Selected model with {selected_model.n_topic} topics")
    else:
        # If no model returned, find the one with closest topic count
        print(f"[WARN] evaluate_models did not return a model. Selecting manually...")
        ...
        print(f"[OK] Selected model '{closest_label}' with {selected_model.n_topic} topics")
...
def step3_clustering_and_visualization(cistopic_obj):
    ...
    print("\n[STATS] Generating visualization plots...")
    ...
    for var in available_vars:
        try:
            ...
            plt.close()
            print(f"   [OK] Saved UMAP colored by {var}")
    ...
    for cluster_col in cluster_cols:
        try:
            ...
            plt.close()
            print(f"   [OK] Saved UMAP colored by {cluster_col}")
        except Exception as e:
            print(f"   [WARN] Could not plot {cluster_col}: {e}")
    
    # Plot topic contributions on UMAP
    print("\n[STATS] Plotting topic contributions...")
    ...
    try:
        ...
        plt.close()
        print(f"   [OK] Saved topic UMAP plots")
    except Exception as e:
        print(f"   [WARN] Could not plot topics: {e}")
    
    # Cell-topic heatmap
    print("\n[STATS] Generating cell-topic heatmap...")
    try:
        ...
        plt.close()
        print("   [OK] Saved cell-topic heatmap")
    except Exception as e:
...
def step4_topic_binarization(cistopic_obj):
    ...
    print(f"\n[STATS] Binarizing topics (top {N_TOP_REGIONS} regions)...")
    ...
    plt.close()
    print(f"   [OK] Binarized {len(region_bin_topics_top_3k)} topics (top {N_TOP_REGIONS})")
    
    # Method 2: Otsu thresholding
    print("\n[STATS] Binarizing topics (Otsu method)...")
    ...
    plt.close()
    print(f"   [OK] Binarized {len(region_bin_topics_otsu)} topics (Otsu)")
    
    # Method 3: Cell-topic binarization (Li method)
    print("\n[STATS] Binarizing cell-topic matrix (Li method)...")
    ...
    plt.close()
    print(f"   [OK] Binarized cell-topic matrix")
    
    # Save region sets to BED files
    print("\n[SAVE] Saving binarized region sets...")
...
def step5_topic_qc(cistopic_obj, binarized_cell_topic):
    ...
    try:
        ...
        plt.close()
        print(f"   [OK] Saved topic QC plots")
    except Exception as e:
        print(f"   [WARN] Could not plot topic QC: {e}")
    
    # Topic annotation by age
    print("\n[STATS] Annotating topics by age...")
    topic_annot = None
    if 'age' in cistopic_obj.cell_data.columns:
        try:
            ...
            print(f"   [OK] Saved topic annotations")
        except Exception as e:
            print(f"   [WARN] Could not annotate topics: {e}")
    else:
        print("   [WARN] 'age' column not found in cell_data")
    
    # Topic annotation by celltype (if available)
    if 'celltype' in cistopic_obj.cell_data.columns:
        print("\n[STATS] Annotating topics by celltype...")
        try:
            ...
            print(f"   [OK] Saved celltype topic annotations")
        except Exception as e:
            print(f"   [WARN] Could not annotate topics by celltype: {e}")
...
def step6_differential_accessibility(cistopic_obj):
    ...
    print(f"   [OK] Imputation complete. Shape: {imputed_acc_obj.X.shape}")
    
    # Normalize scores
    print("\n[STATS] Normalizing imputed scores...")
    ...
    plt.close()
    print(f"   [OK] Found {len(variable_regions)} highly variable regions")
    ...
    print(f"   [OK] Saved highly variable regions to BED file")
    
    # Find differentially accessible regions by age
    markers_dict = None
    if 'age' in cistopic_obj.cell_data.columns:
        print("\n[STATS] Finding differentially accessible regions by age...")
        try:
            ...
            # Save DAR results
            print("\n[SAVE] Saving differentially accessible regions...")
            ...
            print(f"   [OK] Saved DARs for {len(markers_dict)} age groups")
            
            # Print summary
            print("\n[STATS] DAR Summary by Age Group:")
            for group_name, dar_df in markers_dict.items():
                ...
                print(f"   {group_name}: {len(dar_df)} DARs (^{n_up}, v{n_down})")
                
        except Exception as e:
            print(f"   [WARN] Error finding DARs: {e}")
    else:
        print("   [WARN] 'age' column not found - skipping DAR analysis")
    
    # Find DARs by celltype (if available)
    if 'celltype' in cistopic_obj.cell_data.columns:
        print("\n[STATS] Finding differentially accessible regions by celltype...")
        try:
            ...
            print(f"   [OK] Saved DARs for {len(markers_dict_celltype)} cell types")
            
        except Exception as e:
            print(f"   [WARN] Error finding celltype DARs: {e}")
...
def step7_plot_imputed_features(cistopic_obj, imputed_acc_obj, markers_dict):
    ...
    if markers_dict is None:
        print("[WARN] No markers dict available - skipping feature plots")
        return
    ...
    for group_name, dar_df in markers_dict.items():
        ...
        if len(top_regions) > 0:
            try:
                ...
                plt.close()
                print(f"   [OK] Saved imputed feature plots for {group_name}")
            except Exception as e:
                print(f"   [WARN] Could not plot features for {group_name}: {e}")
