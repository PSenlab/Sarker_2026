...
path_to_blacklist = "path/to/mm10-blacklist.v2.bed"

# Cell annotation file path (for hepatocytes or other cell types)
CELL_DATA_ANNOTATION_PATH = "cell_data_Heps.tsv"

# pycistopic binary - update to the absolute path inside your conda env,
# or leave as "pycistopic" if it is on your PATH
PYCISTOPIC_BIN = "pycistopic"

# Base directory containing per-sample Cell Ranger ARC output folders.
# Script expects fragments at:
#   {CELLRANGER_BASE_DIR}/{folder_name}/{folder}/outs/atac_fragments.tsv.gz
CELLRANGER_BASE_DIR = "path/to/CellRanger"
...
def load_and_process_cell_data(fragments_dict):
    ...
    print(f"[STATS] Loaded cell_data with shape: {cell_data.shape}")
    ...
    print(f"[OK] Final filtered sample_ids: {len(valid_sample_ids)}")
    ...
    if not filtered_fragments_dict:
        raise ValueError("[ERROR] Filtered fragments_dict is empty. Check your filtering logic.")
...
def get_consensus_peaks_wrapper(narrow_peaks_dict, chromsizes):
    ...
    try:
        ...
        print(f"[OK] Saved consensus peaks to {regions_bed_filename}")
...
def get_tss_annotations():
    ...
    try:
        ...
        print(f"[OK] TSS annotations saved to {tss_bed_filename}")
    except subprocess.CalledProcessError as e:
...
def run_qc(filtered_fragments_dict):
    ...
    for sample_id in filtered_fragments_dict:
        ...
        if not os.path.exists(sample_metrics_path):
            print(f"[ERROR] File still missing: {sample_metrics_path}")
        else:
            print(f"[OK] Verified: {sample_id}")
...
def generate_qc_plots(filtered_fragments_dict, sample_id_to_barcodes_passing_filters, sample_id_to_thresholds):
    ...
    print(f"[OK] Saved QC summary stats to {os.path.join(qc_plots_dir, 'qc_summary_stats.csv')}")
    ...
    plt.close(fig)
    print(f"[OK] Saved cells per sample plot")
    ...
    if all_metrics:
        ...
        if 'tss_enrichment' in combined_metrics.columns:
            ...
            plt.close(fig)
            print(f"[OK] Saved TSS enrichment by age group plot")
        ...
        if 'fraction_of_fragments_in_peaks' in combined_metrics.columns:
            ...
            plt.close(fig)
            print(f"[OK] Saved FRIP by age group plot")
        ...
        if 'tss_enrichment' in combined_metrics.columns and 'unique_fragments_count' in combined_metrics.columns:
            ...
            plt.close(fig)
            print(f"[OK] Saved TSS vs fragments scatter plot")
    
    print(f"\n[OK] All QC plots saved to {qc_plots_dir}")
...
def create_cistopic_objects(filtered_fragments_dict, sample_id_to_barcodes_passing_filters):
    ...
    for sample_id in filtered_fragments_dict:
        ...
        if not os.path.exists(sample_metrics_path):
            print(f"[WARN] Metrics file missing for {sample_id}: {sample_metrics_path}")
            continue
        ...
        print(f"   [STATS] {sample_id}: {n_cells_before} cells -> {n_cells_after} singlets ({n_doublets} doublets removed)")
...
def merge_cistopic_objects(cistopic_obj_list):
    ...
    with open(merged_path, 'wb') as f:
        pickle.dump(cistopic_obj, f)
    print(f"[OK] Saved merged cisTopic object to {merged_path}")
...
def add_cell_annotations(cistopic_obj, annotation_file_path=None):
    ...
    if not os.path.exists(annotation_file_path):
        print(f"[WARN] Annotation file not found: {annotation_file_path}")
    ...
    if len(non_one_barcodes) > 0:
        print(f"[WARN] Found {len(non_one_barcodes)} barcodes with suffix other than -1")
    ...
    print(f"[WARN] Dropped {before_drop - len(cell_data)} rows with invalid index transformation")
    ...
    print(f"[OK] Transformed annotation index. Sample entries:\n   {cell_data.index[:3].tolist()}")
    ...
    print(f"   [ERROR] In annotation but not in cisTopic: {len(missing_from_cistopic)}")
    print(f"   [ERROR] In cisTopic but not in annotation: {len(missing_from_annotation)}")
    
    if len(intersect) == 0:
        print("[ERROR] No matching barcodes found! Check barcode format.")
        return cistopic_obj
    ...
    if len(duplicates_in_cell_data) > 0:
        print(f"[WARN] Found {len(duplicates_in_cell_data)} duplicate indices in annotation data")
    ...
    if len(duplicates_in_cistopic) > 0:
        print(f"[WARN] Found {len(duplicates_in_cistopic)} duplicate indices in cisTopic object")
    ...
    nan_count = cistopic_obj.cell_data.isna().sum().sum()
    if nan_count > 0:
        print(f"[WARN] Found {nan_count} NaN values. Cleaning...")
        cistopic_obj.cell_data = cistopic_obj.cell_data.dropna()
    
    print(f"[OK] Final annotated cell_data shape: {cistopic_obj.cell_data.shape}")
...
def filter_and_finalize(cistopic_obj, required_columns=None):
    ...
    if missing_required:
        print(f"[WARN] Missing expected columns: {missing_required}")
    
    if existing_required:
        print(f"[INFO] Filtering on columns: {existing_required}")
    ...
    print(f"\n[STATS] Total unique samples: {len(sample_counts_dict)}")
    print(f"[STATS] Total cells: {sum(sample_counts_dict.values())}")
...
def print_final_summary(cistopic_obj):
    ...
    print("\n" + "="*60)
    print("[STATS] FINAL OBJECT SUMMARY FOR SCENIC+")
    print("="*60)
    ...
    print(f"\n[OK] Final number of cells with complete metadata: {complete_meta.shape[0]}")
    ...
    if missing_cols:
        print(f"[WARN] Warning: Missing recommended columns for SCENIC+: {missing_cols}")
    else:
