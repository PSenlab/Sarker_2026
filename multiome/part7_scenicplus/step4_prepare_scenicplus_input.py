...
# Input file paths
RNA_ANNDATA_PATH = "/data/sarkern2/multiome_liver/final_object/final_rna_wnn.h5ad"
...
def load_rna_anndata(filepath: str, cell_type: Optional[str] = None) -> ad.AnnData:
    ...
    if cell_type is not None:
        ...
        print(f"   ✅ Filtered to {adata.n_obs} {cell_type} cells")
...
def load_cistopic_object(filepath: str):
    ...
    print(f"   ✅ Loaded {n_cells} cells x {n_regions} regions")
...
def preprocess_rna(adata: ad.AnnData, target_sum: float = 1e4) -> ad.AnnData:
    ...
    print(f"   ✅ Normalized to target sum {target_sum:.0e}")
...
def transform_rna_indices(adata: ad.AnnData) -> ad.AnnData:
    ...
    if failed_indices:
        print(f"   ⚠️ {len(failed_indices)} indices could not be transformed")
        print(f"      Examples: {failed_indices[:3]}")
    
    adata.obs_names = pd.Index(transformed_indices)
    print(f"   ✅ Transformed {len(transformed_indices)} indices")
...
def remove_duplicates(adata: ad.AnnData, cistopic_obj) -> Tuple[ad.AnnData, object]:
    ...
    if n_rna_dups > 0:
        print(f"   ⚠️ Found {n_rna_dups} duplicate indices in RNA data")
        adata = adata[~rna_dups].copy()
        print(f"   ✅ Removed RNA duplicates: {adata.n_obs} cells remaining")
    ...
    if n_cistopic_dups > 0:
        print(f"   ⚠️ Found {n_cistopic_dups} duplicate indices in cisTopic data")
        cistopic_obj.cell_data = cistopic_obj.cell_data[~cistopic_dups]
        print(f"   ✅ Removed cisTopic duplicates: {len(cistopic_obj.cell_data)} cells remaining")
...
def find_matching_cells(adata: ad.AnnData, cistopic_obj) -> Tuple[ad.AnnData, object, set]:
    ...
    print(f"   ✅ Shared cells: {len(shared_indices)}")
    print(f"   ❌ In RNA only: {len(missing_from_atac)}")
    print(f"   ❌ In ATAC only: {len(missing_from_rna)}")
...
def subset_to_shared_cells(adata: ad.AnnData, cistopic_obj, shared_indices: set) -> Tuple[ad.AnnData, object]:
    ...
    print("   ✅ Indices aligned and verified")
...
def set_categorical_age(adata: ad.AnnData, cistopic_obj, age_order: List[str]) -> Tuple[ad.AnnData, object]:
    ...
    if 'age' in adata.obs.columns:
        ...
        print(f"   ✅ RNA age categories: {adata.obs['age'].cat.categories.tolist()}")
    else:
        print("   ⚠️ 'age' column not found in RNA data")
    ...
    if 'age' in cistopic_obj.cell_data.columns:
        ...
        print(f"   ✅ cisTopic age categories: {cistopic_obj.cell_data['age'].cat.categories.tolist()}")
    else:
        print("   ⚠️ 'age' column not found in cisTopic data")
...
def save_outputs(adata: ad.AnnData, cistopic_obj, gex_path: str, cistopic_path: str):
    ...
    print(f"   ✅ Saved GEX AnnData: {gex_path}")
    
    # Save cisTopic object
    with open(cistopic_path, "wb") as f:
        pickle.dump(cistopic_obj, f)
    print(f"   ✅ Saved cisTopic object: {cistopic_path}")
...
def print_final_summary(adata: ad.AnnData, cistopic_obj):
    ...
    # Verify alignment
    print(f"\n✅ Index alignment verified: {list(adata.obs_names[:3])} ...")
